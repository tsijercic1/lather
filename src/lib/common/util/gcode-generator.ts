...
const MAX_A_AXIS = 89_999.000;

export interface GenerateGCodeOptions {
	toolNumber: number;
	feedRate: number;
	mmPerRotation: number;
	spindleSpeed: number;
	homeZ: number;
}
...
export default function generateGCode(points: Point2D[], options: GenerateGCodeOptions): string {
	points = insertPointsBetweenFarPoints(points, 5);
	let lineCounter = 0;
	const travel = options.mmPerRotation;
	...
	for (let i = 1; i < points.length; i++) {
		...
		if (aAxis + aAxisIncrement * aAxisSign > MAX_A_AXIS || aAxis + aAxisIncrement * aAxisSign < 0) {
			aAxisSign *= -1;
		}
